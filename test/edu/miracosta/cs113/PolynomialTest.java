...
public class PolynomialTest {
	...
	// TERMS + POLYNOMIALS with unique terms (no duplicate terms = terms with same
	// exponents)
	private static final Term[] UNIQUE_TERMS1 = { new Term(5, 7), new Term(5, 9), new Term(-2, 2), new Term(-4, -3) };
	private static final String[] UNIQUE_POLY1 = { "5x^7", "5x^9 + 5x^7", "5x^9 + 5x^7 - 2x^2",
			"5x^9 + 5x^7 - 2x^2 - 4x^-3" };
	private static final int[] UNIQUE_FINAL_ORDER1 = { 1, 0, 2, 3 };

	private static final Term[] UNIQUE_TERMS2 = { new Term(15, 5), new Term(145, 9), new Term(-5, 7) };
	private static final String[] UNIQUE_POLY2 = { "15x^5", "145x^9 + 15x^5", "145x^9 - 5x^7 + 15x^5" };
	private static final int[] UNIQUE_FINAL_ORDER2 = { 1, 2, 0 };

	private static final Term[] UNIQUE_ADD_ORDER = { new Term(150, 9), new Term(15, 5), new Term(-2, 2),
			new Term(-4, -3), };

	// TERMS + POLYNOMIALS with unique terms (no duplicate terms = terms with same
	// exponents)
	private static final Term[] DUPLICATE_TERMS = { new Term(-12, 11), new Term(3, 13), new Term(-1, 4), new Term(9, 6),
			new Term(52, 8), new Term(-88, 10), new Term(11, 12), new Term(7, 2), new Term(-1, -1), new Term(4, 1),
			new Term(12, 0), new Term(100, 5), new Term(100, 10), // duplicate
	...
	private static final String[] DUPLICATE_POLY = { "- 12x^11", "3x^13 - 12x^11", "3x^13 - 12x^11 - x^4",
			"3x^13 - 12x^11 + 9x^6 - x^4", "3x^13 - 12x^11 + 52x^8 + 9x^6 - x^4",
			"3x^13 - 12x^11 - 88x^10 + 52x^8 + 9x^6 - x^4", "3x^13 + 11x^12 - 12x^11 - 88x^10 + 52x^8 + 9x^6 - x^4",
	...
	private static final Term[] DUPLICATE_FINAL_ORDER = { new Term(1, 13), new Term(11, 12), new Term(-12, 11),
			new Term(12, 10), new Term(2, 8), new Term(9, 6), new Term(100, 5), new Term(-1, 4), new Term(7, 2),
			new Term(4, 1), new Term(2, 0), new Term(-2, -1), };
	...
	public void testToStringUnique1() {
		...
		for (int i = 0; i < UNIQUE_TERMS1.length; i++) {
			...
			// remove whitespace from Strings, more flexible testing (doesn't depend on
			// whitespace matching)
	...
	public void testDuplicateAddTermTypeBySizeOfList() {
		...
		for (int i = finalSize; i < DUPLICATE_TERMS.length; i++) {
			test.addTerm(DUPLICATE_TERMS[i]);
			assertEquals("Term was added but DIDN'T combine like terms (matching exponents", finalSize,
					test.getNumTerms());
		}
	}
	...
	/**
	 * TODO only allowed to change this file for EXTRA CREDIT add tests for: -
	 * duplicate add terms by string (not Term objects), - duplicate toString()
	 * tests - anything else you can think of testing!
	 */
}